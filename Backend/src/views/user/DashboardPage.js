...
import { processClear, processList } from '../../actions/user.actions';
...
function DashboardPage(props) {
  ...
  /* eslint-disable */
    ...
    const loadData = async () => {
      ...
      let res = await props.processList('map-projects', {}, true);
      if(res && res.result && res.result.length){
        await props.processList('map-layers', {}, true);
          ...
          <>
            <Breadcrumb 
              ...
              structure={[
                ...
                { title: 'ข้อมูลแผนที่', to: '/user' }
              ]}
            />
            <div className="grids">
              {!props.list.length? (
                <div className="grid sm-100">
                  <div className="text-center" style={{ paddingTop: '3rem', paddingBotttom: '3rem' }}>
                    <h6 className="fw-600">ไม่พบข้อมูลแผนที่ของคุณ</h6>
                  </div>
                </div>
              ): (
                props.list.map((d, i) => (
            ...
            <div className="app-card">
              <div className="breadcrumb">
                ...
                <div className="structure color-gray">
                  <Link className="h-color-p" to="/user">สำหรับผู้ใช้งาน</Link>
                  <div className="ml-1 mr-1">/</div> 
                  <div className="color-p">ข้อมูลแผนที่</div>
                </div>
              </div>
            </div>
            {!mapData.isValid()? (
