...
import { MapDataModel, AddressModel } from '../../models';


function MapDataPage(props) {
  const history = useNavigate();
  const params = useParams();
  const process = params.process? params.process: 'create';
  const dataId = params['*']? params['*']: null;

  const [values, setValues] = useState(new MapDataModel({ status: 1 }));
  ...
  const onSubmit = async (e) => {
    ...
    if(process === 'create'){
      ...
      if(res) history(`/admin/map-datas`);
    }else if(process === 'update'){
      let updateInput = { ...values, mapLocation: address };
      await props.processUpdate('map-data', updateInput, true);
    }
  };
  ...
  /* eslint-disable */
    ...
    if(['create', 'view', 'update'].indexOf(process) < 0){
      history('/admin/map-datas');
    }else{
      if(['view', 'update'].indexOf(process) > -1){
        props.processRead('map-data', { _id: dataId }, true).then(d => {
          setValues(d);
          setAddress(d.mapLocation);
        }).catch(() => history('/admin/map-datas'));
      }
    }
  }, []);
  /* eslint-enable */
    ...
    <div className="app-container">
      <Breadcrumb 
        ...
        structure={[
          ...
          { title: 'Map Data', to: '/admin/map-datas' }
        ]}
      />
      ...
      <div className="app-card p-0 mt-4">
        <form onSubmit={onSubmit}>
          ...
          <div className="app-card-block">
            <p className="lg fw-800">ข้อมูล GeoJSON</p>
            <div className="ss-sep-01 mt-3"></div>
          ...
          <div className="app-card-block border-top-1 bcolor-fgray pt-0">
            <div className="btns">
              ...
              {process === 'update'? (
                <Link to={`/admin/map-data/view/${dataId}`} className="btn btn-action btn-p-border">
                  ดูข้อมูล
                </Link>
              ): (<></>)}
              <Link to="/admin/map-datas" className="btn btn-action btn-default">
                ย้อนกลับ
              </Link>
            </div>
          </div>
...
MapDataPage.defaultProps = {
	
};
MapDataPage.propTypes = {
